...
### Sending Mail Regionally

The regional mail-send API requires sending requests to the specific regional DNS endpoint.
You can achieve this either by setting the host, through the `setHost(...)` method, which
requires knowing the hostname you'd like to set, or by setting the region through the `setRegion(...)` method.
You'll notice the below example is identical to the one above, but includes setting the region.
